...
function assetsCopy() {
  return src([
    ...
    path.to.srcAsset.imageFiles + "/*.ico",
  ]).pipe(dest(path.to.siteAssets.assetFilesSite));
}

// 'gulp copy:images' -- copies unoptimized images to /dist/
function imagesCopy() {
  return src([path.to.fileGlob.imageFilesGlob]) // do not process feature images
...
function iconsCopy() {
  return src(path.to.srcAsset.imageFiles + "/*.ico")
    .pipe(newer(path.to.siteAssets.imageFilesSite))
    .pipe(dest(path.to.siteAssets.imageFilesSite));
}
...
function manifestCopy() {
  return src(path.to.srcAsset.imageFiles + "/*.json")
    .pipe(newer(path.to.siteAssets.imageFilesSite))
    .pipe(dest(path.to.siteAssets.imageFilesSite));
}
...
function imagesCopyCached() {
  if (process.env.CONTEXT === "production") {
    return src("/opt/build/cache/assets/images" + "/**/*")
      .pipe(newer(path.to.siteAssets.imageFilesSite))
      .pipe(dest(path.to.siteAssets.imageFilesSite));
  } else {
    return src(path.to.tmpAssets.imageFilesTemp + "/**/*")
      .pipe(newer(path.to.siteAssets.imageFilesSite))
      .pipe(dest(path.to.siteAssets.imageFilesSite));
  }
}
