...
async function images(src, alt, classParent, classDescendent, sizes) {
  ...
  if (sizes === "card") {
    newSizes =
      "(min-width: 1260px) 470px, (min-width: 620px) calc(42.1vw - 52px), calc(92.33vw - 64px), 100vw";
    ...
    ${Object.values(metadata)
      .map((imageFormat) => {
        return `<source type="${imageFormat[0].sourceType}"
          srcset="${imageFormat.map((entry) => entry.srcset).join(", ")}" sizes="${newSizes}">`;
      })
      .join("\n")}
      <img
